...
import { IdempotencyService } from "@application/services/idempotency/idempotency-check.service";
...
export const ExternalModule = new ContainerModule(({ bind }) => {
    ...
    bind<IIdempotencyService>(EXTERNAL_TYPES.IdempotencyService).to(IdempotencyService)
});
