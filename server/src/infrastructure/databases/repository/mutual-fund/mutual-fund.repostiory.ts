...
export class MutualFundRepository extends BaseRepository<MutualFundEntity, MutualFundDocument> implements IMutualFundRepository {
    ...
    async findActiveFunds(options?: QueryOptions): Promise<{ funds: MutualFundEntity[], countActiveFunds: number }> {
        let docs = null;
        if (options) {
            const {
                page = 1,
            ...
            const finalFilter: FilterFund = { ...filter, status: "Active" };
        ...
        return {
            funds: docs ? docs.map(doc => this.mapper.toDomain(doc)) : [],
