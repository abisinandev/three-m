...
import type { IForgotPasswordResendOtpUseCase } from "../auth/interfaces/forgot-pass-resend-otp-usecase.interface";
  ...
  async execute(data: ResendOtpDTO): Promise<ResendOtpResponseDTO> {
    ...
    if (otpData?.otp) {
      if (otpData.lastResendAt && now - Number(otpData.lastResendAt) < 30000) {
        throw new AppError(
          ErrorMessages.AUTH.RATE_LIMIT_MESSAGE,
      ...
      if (otpData.resendCount && Number(otpData.resendCount) >= 5) {
        throw new AppError(
          ErrorMessages.AUTH.MAX_RESEND_REACHED,
          HttpStatus.TOO_MANY_REQUESTS,
        );
      }
