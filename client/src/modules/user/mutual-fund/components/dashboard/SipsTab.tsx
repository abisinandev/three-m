    ...
    return (
        <div className="bg-[#111214] border border-[#1e2025] rounded-xl p-6 space-y-6 shadow-2xl">
            <div className="flex justify-between items-center">
                <h3 className="text-sm font-semibold flex items-center gap-2 text-gray-200 uppercase tracking-wider">
                    <CalendarCheck size={16} className="text-green-500" />
                    Active SIPs
                </h3>
                <span className="text-xs bg-[#1e2025] px-2.5 py-1 rounded-full text-gray-400 font-bold uppercase tracking-wider">
            ...
            {sipsLoading ? (
                <div className="py-20 text-center">
                    ...
                    <p className="text-xs text-gray-500 uppercase tracking-widest font-bold">Synchronizing SIP Data...</p>
                </div>
            ) : sips.length === 0 ? (
                        ...
                        <div key={sip.id} className="group relative border border-[#1e2025] hover:border-[#2a2d35] rounded-2xl p-5 bg-[#0d0d0e] transition-all duration-300">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-semibold text-gray-200 line-clamp-1 mb-1 group-hover:text-green-400 transition-colors">
                                            {sip.schemeName || sip.schemeCode}
                                        </h4>
                                        <div className="flex items-center gap-2 text-xs font-medium text-gray-400">
                                            <span className="text-gray-200 font-semibold tabular-nums">₹{sip.amount.toLocaleString('en-IN')}</span>
                                            <span>•</span>
                                            <span className="uppercase">{sip.frequency}</span>
                                            <span>•</span>
                                            <span className="tabular-nums">{dayjs(sip.startDate).format('MMM D, YYYY')}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-6 bg-[#111214] md:bg-transparent p-3 md:p-0 rounded-xl border border-[#1e2025] md:border-0">
                                    <div className="text-right">
                                        <p className="text-xs text-gray-500 font-medium mb-1">Next Installment</p>
                                        <div className="flex items-center justify-end gap-1.5 text-xs font-semibold text-green-400 tabular-nums">
                                            <Clock size={12} />
                            ...
                            <details className="mt-5 border-t border-[#1e2025] pt-3 group">
                                <summary className="text-xs font-semibold text-gray-400 cursor-pointer hover:text-gray-300 list-none flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <ChevronDown size={14} className="group-open:rotate-180 transition-transform" />
                                        Payment History ({sip.installments?.length || 0})
                                    </div>
                                    {sip.executedInstallments > 0 && (
                                        <span className="text-green-500/80 tabular-nums">{sip.executedInstallments} Successful</span>
                                    )}
                                </summary>
                                <div className="mt-4 grid gap-2">
                                        ...
                                        <div key={inst.id || idx} className="flex flex-col bg-[#111214] p-3 rounded-xl border border-[#1e2025] gap-1.5">
                                            <div className="flex justify-between items-center w-full">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-xs font-medium text-gray-400 tabular-nums">{dayjs(inst.executionDate).format('DD MMM YYYY')}</span>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <span className="text-xs font-semibold text-gray-200 tabular-nums">₹{inst.amount.toLocaleString('en-IN')}</span>
                                                    <span className={`text-xs font-medium uppercase px-2 py-0.5 rounded-md ${inst.status === 'SUCCESS' ? 'bg-green-500/10 text-green-500' :
                                            ...
                                            {inst.status === 'FAILED' && inst.failureReason && (
                                                <div className="flex items-center gap-1.5 text-xs text-red-400 bg-red-500/5 px-2 py-1 rounded-lg border border-red-500/10 self-start">
                                    ...
                                    {(!sip.installments || sip.installments.length === 0) && (
                                        <div className="py-4 text-center text-xs text-gray-600 font-bold uppercase tracking-widest">No transaction history yet</div>
                                    )}
                                </div>
                            </details>
                        </div>
