        ...
        <div className="space-y-6">
            <div className="bg-[#111214] border border-[#1e2025] rounded-lg p-4 shadow-sm">
                <div className="flex items-center justify-between pb-4 border-b border-[#1e2025] mb-4">
                    <h3 className="text-sm font-semibold text-[#e8eaed] flex items-center gap-2 m-0 leading-none">
                    ...
                    {recentNAVUpdates.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center py-2 border-b border-[#1e2025] last:border-0 hover:bg-[#1a1c20] px-2 -mx-2 rounded transition-colors group">
                            <div className="truncate flex-1 pr-2">
                                <p className="text-sm font-semibold text-[#e8eaed] truncate transition-colors">
                                    {item.fund}
                                </p>
                                <p className="text-xs text-[#aab0c0] font-mono mt-1 tabular-nums">
                                    ₹{item.nav.toFixed(2)}
                                </p>
                            </div>
                            <div className="text-right">
                                <p className={`text-xs font-semibold font-mono tabular-nums ${item.change >= 0 ? 'text-[#00C853]' : 'text-[#FF1744]'}`}>
                                    {item.change >= 0 ? '+' : ''}{item.change.toFixed(2)}%
                                </p>
                                <p className="text-xs text-[#5a5f6e] mt-1 font-medium tabular-nums">{item.date}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-5 pt-3 border-t border-[#1e2025]">
                    <p className="text-xs text-[#5a5f6e] text-center font-medium">
