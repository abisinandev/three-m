...
const StatCol = ({
    ...
    return (
        <div className={`flex flex-col ${plain ? 'pl-0 border-none' : 'pl-4 sm:pl-6 border-l border-[#1e2025]'}`}>
            <p className="text-xs text-[#5a5f6e] tracking-wider uppercase mb-1 m-0">
                {label}
            </p>
            <p className={`text-sm font-bold flex items-center gap-1 m-0 tabular-nums ${colorClass}`}>
            ...
            {sub && (
                <p className={`text-xs mt-1 m-0 tabular-nums ${positive === undefined ? 'text-[#5a5f6e]' : colorClass}`}>
                    {sub}
                </p>
            )}
        </div>
...
export const SummaryStats = ({
    ...
    return (
        <div className="bg-[#111214] border border-[#1e2025] rounded-lg p-5 sm:p-6 shadow-sm">
            <div className="mb-6">
                <p className="text-xs font-semibold text-[#5a5f6e] tracking-wider uppercase mb-1 m-0">
                    Portfolio Value
                </p>
                {isLoading ? (
                    <div className="h-9 w-48 bg-[#1e2025] rounded animate-pulse" />
                ) : (
                    <p className="text-4xl font-bold text-[#e8eaed] tracking-tight leading-none m-0 tabular-nums">
