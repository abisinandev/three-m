import React from 'react';
import { ChevronLeft, ChevronRight } from 'lucide-react';

interface AlgoPaginationProps {
    page: number;
    totalPages: number;
    itemsCount: number;
    totalItems: number;
    unit: string;
    isLoading: boolean;
    onPrevPage: () => void;
    onNextPage: () => void;
}

export const AlgoPagination: React.FC<AlgoPaginationProps> = ({
    page,
    totalPages,
    itemsCount,
    totalItems,
    unit,
    isLoading,
    onPrevPage,
    onNextPage
}) => {
    return (
        <div className="p-4 border-t border-[#1e2025] flex justify-between items-center text-[11px] text-[#5a5f6e] bg-[#111214]">
            <div className="font-medium bg-[#1a1c20] px-2 py-1 rounded">
                Showing <span className="text-white">{itemsCount}</span> of <span className="text-white">{totalItems}</span> {unit}
            </div>
            <div className="flex gap-1.5 items-center">
                <span className="mr-2">Page {page} of {totalPages}</span>
                <button
                    onClick={onPrevPage}
                    disabled={page === 1 || isLoading}
                    className="p-1.5 bg-[#1a1c20] border border-[#1e2025] text-[#5a5f6e] hover:text-white disabled:opacity-30 disabled:hover:text-[#5a5f6e] rounded-md transition-all shadow-sm"
                >
                    <ChevronLeft size={16} />
                </button>
                <button
                    onClick={onNextPage}
                    disabled={page >= totalPages || isLoading}
                    className="p-1.5 bg-[#1a1c20] border border-[#1e2025] text-[#5a5f6e] hover:text-white disabled:opacity-30 disabled:hover:text-[#5a5f6e] rounded-md transition-all shadow-sm"
                >
                    <ChevronRight size={16} />
                </button>
            </div>
        </div>
    );
};
